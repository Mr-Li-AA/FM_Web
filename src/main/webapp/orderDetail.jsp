...
<div class="panel panel-default" style="margin: 0 auto;width: 95%;">
	...
	<table cellpadding="0" cellspacing="0" align="center" width="100%" class="table table-striped table-bordered table-hover">
		...
		<tr>
			<td>送货地址:</td>
			<td>${od.address.proname}省${od.address.cityname}市${od.address.countyname}县${od.address.detail}</td>
			<td>总价:</td>
			<td>${od.order.money/100.00}</td>
		</tr>
		<tr>
				...
				<table align="center" cellpadding="0" cellspacing="0" width="100%"  class="table table-striped table-bordered table-hover">
					...
					<c:forEach items="${od.orderItems}" var="item" varStatus="i">
						<tr align="center">
							<th>${i.count}</th>
							<th>
								<img src="${item.goods.flag==3?"":"/17fmpics/"}${item.goods.picurl}" width="50px" height="50px">
							</th>
							<th>${item.goods.name}</th>
							<th>
								<c:forEach begin="1" end="${item.goods.star}">
									<img src="image/star_red.gif" alt="star"/>
								</c:forEach>
							</th>
							<th>${item.goods.pubdate}</th>
							<th>${item.goods.price/100.00}</th>
							<th>${item.count}</th>
							<th>${item.money/100.00}</th>
						</tr>
					</c:forEach>
				</table>
			</td>
		...
		<tr>
			<td align="right" colspan="4" style="margin-right: 40px;">
				<a href="/orderlist.do" class="btn btn-danger btn-sm">返回订单列表</a>
				&nbsp;&nbsp;
				<c:if test="${od.order.flag eq 0 }">
