...
document.writeln("                    <li>|</li>");
document.writeln("                    <a href=\'userLogout.do\' class=\'a_top\'>注销</a>");
document.writeln("                    <li>|</li>");
document.writeln("                    <a href=\'orderlist.do\' class=\'a_top\'>我的订单</a>");
document.writeln("             </span>");
document.writeln("                <li>|</li>");
document.writeln("                <a href=\'\' class=\'a_top\'>消息通知</a>");
document.writeln("                <a href=\'cartlist.do\' id=\'shorpcar\'>购物车</a>");
document.writeln("            </span>");
...
function init() {
    $.ajax({
        url:'goodstypelist.do?l=2',
        method:'get',
        success:function(data){
            var list = data;

            for(var t in list){
                var a = "<a href='goodsbytid.do?tid="+list[t].id+"'>"+list[t].name+'</a>';
                $('#goodsType').append(a);
            }
        }
    });
    $.get("userlogincheck.do",null,function (obj) {
        console.log(obj);
        if(obj.code==0){
            //登录成功
            $("#sp1").css("display","none");
            $("#sp2").css("display","block");
            $("#au1").html(obj.data.username);
        }else{
            //未登录
